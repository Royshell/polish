...
<template>
  <button
    class="font-pixel cursor-pointer relative overflow-hidden transition-[filter,transform] hover:brightness-125 active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed"
    :class="[
      /* size */
      size === 'sm' ? 'text-[7px] px-2.25 py-1.25'
      : size === 'lg' ? 'text-[11px] px-4.5 py-2.75 w-full'
  ...
  >
    ...
    <span
      ...
      class="absolute top-0 -left-full w-3/5 h-full pointer-events-none [background:linear-gradient(90deg,transparent,rgba(255,255,255,0.15),transparent)] animate-[shine_2.5s_ease-in-out_infinite]"
    />

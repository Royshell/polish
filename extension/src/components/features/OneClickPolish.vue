...
<template>
  <SectionCard title="One-Click Polish" icon="⬛">
    ...
    <div class="relative pb-1.5">
      <span
        class="absolute -top-1.5 right-2.5 text-[16px] text-polish-yellow [text-shadow:0_0_8px_var(--color-polish-yellow)] pointer-events-none z-10 animate-[sparkle_1.5s_ease-in-out_infinite]"
      >
        +✦
      </span>
      ...
      >
        <span
          class="absolute top-0 -left-full w-[60%] h-full pointer-events-none [background:linear-gradient(90deg,transparent,rgba(255,255,255,0.2),transparent)] animate-[shine_3.75s_ease-in-out_infinite]"
        />
        <span
          v-if="isPolishing"
          class="inline-block mr-1 text-polish-green animate-[blink_0.5s_step-end_infinite"
          >█</span
        >
    ...
    <div class="mt-1.5 border-t border-[#111128]">
      <RetroToggle
        ...
        @update:model-value="
          emit('update:toggles', { ...props.toggles, [item.key]: $event })
        "
      />
    </div>
  </SectionCard>
